...
def deployment_detail(id):
    ...
    try:
        ...
        logger.error(f"Deployment detail error: {e}")
...
def new_deployment():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            logger.error(f"New deployment error: {e}")
...
def deployment_progress(id):
    ...
    try:
        ...
        logger.error(f"Deployment progress error: {e}")
...
def execute_deployment_api(id):
    ...
    try:
        ...
        if not deployment:
            logger.warning(f"Deployment {id} not found for user {user_id}")
            return jsonify({
                'success': False, 
        ...
        logger.info(f"🚀 Starting deployment for {name} ({deployment_type}) - User: {user_id}")
        ...
        if result['success']:
            ...
            logger.info(f"✅ Deployment {name} completed successfully")
            return jsonify({
                'success': True,
            ...
            logger.error(f"❌ Deployment {name} failed: {result['output']}")
            return jsonify({
                'success': False,
        ...
        logger.error(f"💥 Execute deployment API error for ID {id}: {e}")
...
def delete_deployment(id):
    ...
    try:
        ...
        logger.error(f"Delete deployment error: {e}")
...
def update_deployment_status(id):
    ...
    try:
        ...
        logger.error(f"Update deployment status error: {e}")
