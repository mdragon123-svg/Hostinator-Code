    ...
    def initialize_database(self):
        ...
        try:
            ...
            try:
                ...
                temp_cursor.execute(f"CREATE DATABASE IF NOT EXISTS {Config.DB_CONFIG['database']} CHARACTER SET {Config.DB_CONFIG['charset']} COLLATE {Config.DB_CONFIG['collation']}")
                temp_cursor.execute(f"USE {Config.DB_CONFIG['database']}")
                temp_conn.commit()
                temp_cursor.close()
                temp_conn.close()
                logger.info(f"✅ Database '{Config.DB_CONFIG['database']}' ensured to exist")
            except mysql.connector.Error as err:
